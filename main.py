...
app.add_middleware(
    CORSMiddleware,
    allow_origins=["*"],  # ⚠️ Allow all origins (use specific URLs in production)
...
async def get_visualization(session_id: str, image_type: str):
    ...
    if image_type not in valid_types:
        return JSONResponse(content={"error": f"Invalid image type. Must be one of: {', '.join(valid_types)}"}, status_code=400)
